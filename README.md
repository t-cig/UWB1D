...
3. Creating and saving all probing files will take several (~20-30GB) of storage. By default, we will not store these probing files for memory efficiency. To store all intermediate probing files, you can add in the `--probestore` flag when running `python3 uwb.py`. With this flag, no probing intermediate files will be saved. Note that since the pipeline execution takes time, with this flag, you need to wait for the overall pipeline to re-run if the session disconnects.
